...
class TpTrackers():
    ...
    def FilterClsTable( self, st, ft, Dep, cyconsts, teachers ):
        ...
        for i in range( 1, rows ):
            ...
            if curteacher not in teachers and len(teachers) >= 1:
                continue
            #  Date Constraint.
        ...
        for c in classes.keys():
            for t in classes[c].keys():
                ...
                teacherlist[t][0] += round( individ_performance, 2 )  # 累积个人业绩。
                teacherlist[t][1] += round(individ_time/60, 2)  # 累积个人课时（单位：小时）。

        # 更新产能课时比。
        for t in teacherlist.keys():
            teacherlist[t][2] = round(teacherlist[t][0] / teacherlist[t][1], 2)

        #  排序教师产值.
        #  对teacherlist（dict）的value排序。
        teacherlist = sorted(teacherlist.items(), key=lambda item: item[1], reverse=True)
        ...
        #  文件不存在则新建。
            ...
            ws = wb.active
        #  写入excel。
        ws.append(["教师姓名", "总产能", "总课时", "产能/课时"])
        for i in range(totaltn):
            #  复杂的dict无法直接输出到excel，先转换成list。
            tmp = [teacherlist[i][0], teacherlist[i][1][0], teacherlist[i][1][1], teacherlist[i][1][2]]
            ws.append(tmp)
        #  保存excel。
        wb.save("Teacher_List.xlsx")


#  Script1. 试算2018.06.01-2019.05-31，所有部门，所有老师的产能。

#  定义functions的parameters。
...
fy19.FilterClsTable( st, ft, Dep, cyconsts, teachers )
