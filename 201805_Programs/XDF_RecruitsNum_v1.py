...
#  18财年的实际业绩比面上业绩
MYSTICAL_RATE =  76500000  / 77677487.85   
...
def LoadClassSchedule( filename ):
    """
    载入配课表,excel文件
    filename为配课表文件名
    return 配课表内容
...
def GetDateRange( table ):
    """
    根据用户输入的时间范围，确定table中的index范围
    table：载入的配课表数据
    return 起始与截止日期，及其index, 该时间范围相当于多少个月
    """
    rows = table.nrows
    ...
    day = (q-p).days
    month_rate = day / 30
    return p, q, start, end, month_rate
    

def GetConstraint():
    """
    没有input参数，通过屏幕获得用户输入参数
    返回Department Constraint 和 Class Type Constraint
    """    
    #  输入要查询的部门
    #  Default为全部部门
...
def NewTeacherPerform( sche_table, entry_table, Dep, cyconsts ):
    """
    ...
    duration(int): 新教师入职duration个月时的产能表现
    """
    perform = {}
    m = entry_table.nrows 
    duration = input("新教师入职x个月的产能表现，请输入x值(Deafault x = 15)：")
...
#    tmp = np.array(tmp)    
    ...
    return NTPerform, round(np.mean(tmp),2) , duration


def NumofTneeded( availableT, gap, estimean ):
    """
    ...
    #  输入目标业绩
#    while True:
#        profit = input("请输入（真实）业绩目标（元）: ")   
#        try:
#            goal = profit / MYSTICAL_RATE # WTF??   
#            break
#        except:
#            print("请输入数字")
    ...
    return goal, totalfee, totalgap, NTnum
...
def Output( startdate, enddate, timerate, Dep, cyconsts, 
    ...
    try:
        ...
        part4N14 = "注：真实业绩 / 面上业绩 的比率为 " + str(round(MYSTICAL_RATE ,2))
        ...
        ws["N11"] = part4N11; ws["N14"] = part4N14  
...
def CalcuNum( sche_table, entry_table, gap_list ):
    while True:
        [ startdate, enddate, start, end, timerate ] = GetDateRange( sche_table )  # 日期范围
        while True:
            ...
            [NTPerform, performMean, duration] = NewTeacherPerform( sche_table, entry_table, Dep, cyconsts )
            #  Step3. 计算需要招聘人数
            estimean = timerate * performMean  #  新教师在所选日期范围内的平均业绩表现
            [goal, totalfee, totalgap, NTnum] = NumofTneeded( availableT, gap, estimean  )
            #  Step4. 输出计算结果
...
if __name__ == '__main__':
    ...
    sche_table = LoadClassSchedule( SCHEDULE_FILENMAE ) 
    entry_table = LoadEntryTime( ENTRYDATE_FILENAME )
